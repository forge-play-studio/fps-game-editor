...
function migrateSceneInstancesToV2(sceneConfig: SceneConfig): void {
  ...
  for (const inst of instances) {
    ...
    // Derive sourceId — keep original modelId format (hyphens) so it matches MODEL_URL_MAP keys
...
function createSceneNodeFromArgs(sceneConfig: SceneConfig, args: ProjectEditorCreateSceneNodeArgs): SceneNodeConfig {
  if (!['group', 'instance', 'transform'].includes(args.kind)) {
    throw new ProjectEditorSceneNodeError(
...
export function addProjectEditorAssetNode(
  ...
  } else if (scene.assets[existingAssetIndex].sourceId !== asset.sourceId) {
    throw new Error(`[ProjectEditor][Document] Scene asset ID conflict: ${asset.id}`);
  }
  ...
  try {
    ...
    if (!rootNode) {
      throw new ProjectEditorSceneNodeError(
        ...
        { nodeId: node.id, assetId: asset.id, sourceId: asset.sourceId },
      );
    }
  } catch (error) {
