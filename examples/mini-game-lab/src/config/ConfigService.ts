...
import economyConfigJson from './economy.json';
...
function normalizeMaterialOverride(value: unknown): MaterialOverrideConfig | undefined {
  ...
  if (typeof value.contrast === 'number' && Number.isFinite(value.contrast)) {
    normalized.contrast = value.contrast;
  }
  if (typeof value.brightness === 'number' && Number.isFinite(value.brightness)) {
    normalized.brightness = value.brightness;
  }
  if (typeof value.saturation === 'number' && Number.isFinite(value.saturation)) {
    normalized.saturation = value.saturation;
  }
  if (typeof value.hue === 'number' && Number.isFinite(value.hue)) {
    normalized.hue = value.hue;
  }
  if (typeof value.colorDensity === 'number' && Number.isFinite(value.colorDensity)) {
    normalized.colorDensity = value.colorDensity;
  }
  if (typeof value.alpha === 'number' && Number.isFinite(value.alpha)) {
    normalized.alpha = value.alpha;
  }
  if (typeof value.alphaCutOff === 'number' && Number.isFinite(value.alphaCutOff)) {
    normalized.alphaCutOff = value.alphaCutOff;
  }
  if (typeof value.transparencyMode === 'number' && Number.isFinite(value.transparencyMode)) {
    normalized.transparencyMode = value.transparencyMode;
  }
...
export class ConfigService {
  ...
  constructor() {
    ...
    this.economyConfig = economyConfigJson as EconomyConfig;
    this.gameConfig = gameConfigJson as GameConfig;

    this.migrateSceneInstancesToV2();
    this.buildIndexes();
  }

  /**
   * Migrate legacy `sceneInstances[]` entries into V2 `scene.assets[]` + `scene.nodes[]`.
   * Called once at construction to handle data written by the platform bridge's legacy save.
   */
  private migrateSceneInstancesToV2(): void {
    const raw = this.sceneConfig as Record<string, any>;
    const instances: any[] = raw.sceneInstances;
    if (!Array.isArray(instances) || instances.length === 0) return;

    if (!this.sceneConfig.scene) {
      (this.sceneConfig as any).scene = { rootId: 'root', assets: [], nodes: [], materials: [], textures: [] };
    }
    ...
    for (const inst of instances) {
      ...
      // Use modelId directly as sourceId/assetId to match MODEL_URL_MAP keys (hyphenated)
      const sourceId = modelId;
      const assetId = modelId;

      if (!existingAssetIds.has(assetId)) {
        scene.assets.push({ id: assetId, type: 'glb', sourceId, defaults: {} } as SceneAssetConfig);
    ...
    console.log(`[ConfigService] Migrated ${instances.length} sceneInstances to V2 format`);
  }
