...
export class SceneBuilder {
  ...
  registerRuntimeModelUrl(modelId: string, url: string): void {
    this.assetLoader.registerRuntimeModelUrl(modelId, url);
  }
  ...
  private buildLayoutPlaceholderSurfaces(): void {
    ...
    for (const overlay of overlays) {
      ...
      if (overlay.textureId && TextureAssets.ground && TextureAssets.ground[overlay.textureId]) {
        const textureUrl = TextureAssets.ground[overlay.textureId];
  ...
  private attachTransformRuntime(nodeConfig: SceneTransformNode, runtimeNode: TransformNode): void {
    ...
    if (nodeConfig.groundDecal.textureId && TextureAssets.ground?.[nodeConfig.groundDecal.textureId]) {
      const texture = new Texture(TextureAssets.ground[nodeConfig.groundDecal.textureId], this.scene);
      texture.hasAlpha = true;
      texture.uScale = -1;
      texture.uOffset = 1;
      if (nodeConfig.groundDecal.textureId === 'conveyor_arrow') {
        texture.level = 2;
      }
      mat.diffuseTexture = texture;
