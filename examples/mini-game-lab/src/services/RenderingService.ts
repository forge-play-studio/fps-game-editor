...
import { SilhouetteOutlineService } from './SilhouetteOutlineService';
...
export class RenderingService {
  ...
  private silhouetteOutlineService: SilhouetteOutlineService | null = null;
  ...
  initialize(cameras: Camera[]): void {
    ...
    if (cameras[0]) {
      this.silhouetteOutlineService = new SilhouetteOutlineService(this.scene);
      this.silhouetteOutlineService.initialize(cameras[0]);
    }
  ...
  private applyImageProcessing(): void {
    ...
    // 启用色彩曲线（用于轻微提升饱和度）
    const colorCurvesCfg = this.config.colorCurves;
    if (colorCurvesCfg?.enabled) {
      this.pipeline.imageProcessing.colorCurvesEnabled = true;
      const curves = new ColorCurves();
      curves.globalSaturation = colorCurvesCfg.globalSaturation;
  ...
  setSilhouetteStaticTargets(meshes: AbstractMesh[]): void {
    this.silhouetteOutlineService?.setStaticTargets(meshes);
  }

  setSilhouetteTargets(meshes: AbstractMesh[]): void {
    this.silhouetteOutlineService?.setTargets(meshes);
  }
   ...
   */
  dispose(): void {
    this.silhouetteOutlineService?.dispose();
    this.silhouetteOutlineService = null;
    if (this.pipeline) {
