...
export interface EditorViewportCameraOptions {
  babylon?: BabylonRuntimeGlobal | null;
  input?: EditorInputControllerApi | null;
  cameraName?: string;
}

export interface EditorViewportCameraCtx {
...
export async function createEditorViewportCamera(
  scene: RuntimeScene,
  sourceCamera: RuntimeCamera | null,
  options: EditorViewportCameraOptions = {},
): Promise<EditorViewportCameraCtx | null> {
  const runtime = getBabylonRuntime(options.babylon);
  const UniversalCamera = runtime?.UniversalCamera;
  const Vector3 = runtime?.Vector3;
  ...
  const camera = new UniversalCamera(options.cameraName ?? 'EditorViewportCamera', startPos, scene) as RuntimeCamera & any;
  ...
  const ctx: EditorViewportCameraCtx = {
    camera,
    canvas,
    renderObs: null,
    onContextMenu: null,
  ...
  if (canvas) {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      const Vector3Ctor = getBabylonRuntime(options.babylon)?.Vector3;
      if (!Vector3Ctor) return;
      const activeButton = ctx.input?.activeViewportNavigationButton() ?? null;
...
export function attachEditorViewportMovement(
  scene: RuntimeScene,
  ctx: EditorViewportCameraCtx,
  options: EditorViewportCameraOptions = {},
): unknown {
  if (ctx.renderObs) return ctx.renderObs;
  const camera = ctx.camera as any;
  const Vector3 = getBabylonRuntime(options.babylon)?.Vector3;
  const input = options.input ?? ctx.input;
  if (!camera || !Vector3 || !input) return null;

  ctx.renderObs = scene.onBeforeRenderObservable.add(() => {
    const activeButton = input.activeViewportNavigationButton();
    const pointerDelta = input.consumeViewportPointerDelta();
    ...
    const pressed = input.pressedViewportMovementKeys();
    if (!input.isViewportNavigationActive() || !pressed.size) return;
...
export function disposeEditorViewportCamera(scene: RuntimeScene, ctx: EditorViewportCameraCtx | null): void {
  if (!ctx) return;
  if (ctx.renderObs) {
...
export function focusEditorViewportSelection(
  camera: RuntimeCamera | null,
  node: RuntimeNode | null,
  options: Pick<EditorViewportCameraOptions, 'babylon'> = {},
): boolean {
  ...
  try {
    ...
    if (typeof editorCamera.setTarget === 'function' && Vector3) {
      ...
      if (isArcRotateCamera) editorCamera.radius = dist;
      return true;
    }
